...
async function getPeople(amount: number): Promise<Result<Person[]>> {
	...
	const personResults = await Promise.all(rawPersonResults.data.map(async (person, i) => {
		...
		const personEntity = new Person(i, person.name, person.surname, person.gender, cprResult.data, address, phoneNumber, birthdate)

		return { type: "ok", data: personEntity };
	}))

	const personErrResult = personResults.find(personResult => personResult.type === "err")
	if (personErrResult) {
